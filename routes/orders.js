...
router.post('/create', auth, async (req, res) => {
  try {
    ...
    if (addressId) {
      ...
      if (!addr) return res.status(400).json({ message: 'Shipping address not found' });
      ship = addr.toObject();
      delete ship._id;
    }
    ...
    const order = await Order.create({ user: req.user.id, items, shippingAddress: ship, totalAmount, status: 'pending', paymentStatus: 'pending', paymentProvider: 'stripe' });
  ...
  res.status(201).json({ orderId: order._id, sessionId: session.id, sessionUrl: session.url });
  } catch (err) {
    console.error(err);
